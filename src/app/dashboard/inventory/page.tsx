...
export default function InventoryPage() {
  ...
  const [recordForIncubation, setRecordForIncubation] =
  ...
  useEffect(() => {
    try {
      ...
      if (savedRecords) {
        ...
        // No saved records, start with empty array
        setInventoryRecords([]);
      }
    } catch (error) {
      console.error(
        "Error loading inventory records from localStorage:",
        error
      );
    }
  }, []);
  ...
  const [chartType, setChartType] = useState<"7days" | "30days" | "weekly">(
    "7days"
  );
  ...
  const handleInputChange = (
    ...
    if (id === "eggCount" || id === "broken" || id === "incubated") {
      ...
      return setFormData((prev) => ({ ...prev, [id]: numericValue }));
    }
  ...
  const getFilteredRecords = () =>
    inventoryRecords.filter((record: InventoryRecord) => {
      // Search term filter
      const matchesSearch =
        searchTerm === "" ||
      ...
      // Method filter
      const matchesMethod =
        methodFilter === "All Methods" || record.pickupMethod === methodFilter;
  ...
  const handleConfirmDelete = () => {
    if (recordToDelete) {
      ...
      const updatedRecords = inventoryRecords.filter(
  ...
  const totalThisWeek = thisWeekRecords.reduce(
    (sum: number, record: InventoryRecord) => sum + record.eggCount,
    0
  );
  const dailyAverage =
    thisWeekRecords.length > 0
      ? (totalThisWeek / thisWeekRecords.length).toFixed(1)
      : "0";
  ...
  const totalLastWeek = lastWeekRecords.reduce(
    (sum: number, record: InventoryRecord) => sum + record.eggCount,
    0
  );
  const trend =
  ...
  const handleConfirmIncubation = () => {
    if (
      !recordForIncubation ||
      incubationData.eggCount <= 0 ||
      !incubationData.speciesVariety
    ) {
    ...
    try {
      ...
      const today = new Date();
      const speciesInfo: Record<
        string,
        { incubationDays: number; temperature: number; humidity: number }
      > = {
      ...
      hatchDate.setDate(
        today.getDate() + speciesInfo[incubationData.speciesType].incubationDays
      );
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-white bg-opacity-95 backdrop-blur-sm rounded-2xl p-6 mb-6 shadow-lg border border-white border-opacity-20">
              ...
              <p className="text-amber-800">
                Monitor your daily egg production with smart insights and
                analytics
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
              <div className="bg-white bg-opacity-90 backdrop-blur-sm rounded-xl p-6 text-center shadow-md border border-white border-opacity-30 transform transition hover:translate-y-[-5px] hover:shadow-lg">
                ...
                <div className="text-sm font-medium uppercase tracking-wide text-amber-800">
                  Eggs This Week
                </div>
              </div>
              <div className="bg-white bg-opacity-90 backdrop-blur-sm rounded-xl p-6 text-center shadow-md border border-white border-opacity-30 transform transition hover:translate-y-[-5px] hover:shadow-lg">
                <div className="text-4xl font-bold text-green-600 mb-2">
                  {Math.floor(availableDozens.total)}
                </div>
                <div className="text-sm font-medium uppercase tracking-wide text-green-800">
                  Dozens Available
                </div>
                ...
                <form id="egg-form" onSubmit={handleSubmit}>
                  ...
                  <div className="mb-4">
                    ...
                    >
                      ...
                      <option value="Morning Coop Check">
                        Morning Coop Check
                      </option>
                      <option value="Evening Collection">
                        Evening Collection
                      </option>
                      <option value="Free Range Pickup">
                        Free Range Pickup
                      </option>
                      <option value="Nest Box Collection">
                        Nest Box Collection
                      </option>
                    </select>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <input
                        ...
                        id="incubated"
                        value={
                          formData.incubated === 0 ? "" : formData.incubated
                        }
                        onChange={handleInputChange}
                        placeholder="0"
                        min="0"
              ...
              <div className="bg-white bg-opacity-95 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-white border-opacity-20">
                ...
                <div className="mb-4">
                  ...
                  <div className="text-xs text-gray-500 mt-2 text-center">
                    {chartType === "7days"
                      ? "Daily egg production for the last 7 days"
                      : chartType === "30days"
                      ? "Daily production with 7-day moving average"
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 mb-4">
                        ...
                        <div className="text-sm">
                          ...
                          {record.eggCount - record.incubated - record.broken}
                        </div>
                      </div>
                      {record.notes && (
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          onClick={() => {
                            ...
                            const recordToEdit = inventoryRecords.find(
                              (r) => r.id === record.id
                            );
                            if (recordToEdit) {
                              setFormData({
                              ...
                              // Scroll to form
                              const formElement =
                                document.getElementById("egg-form");
                              if (formElement) {
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Maximum: {recordForIncubation.incubated} eggs
                </p>
              </div>
              ...
              <div>
                ...
                >
                  ...
                  {incubationData.speciesType === "goose" &&
                    ["Embden", "Toulouse", "Chinese", "African", "Pilgrim"].map(
                      (variety) => (
                        <option key={variety} value={variety}>
                          {variety}
                        </option>
