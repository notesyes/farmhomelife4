  ...
  const handleDownloadInvoice = (customer: Customer) => {
    ...
    if (typeof window !== "undefined" && window.jspdf) {
      try {
        ...
        doc.text(
          `Invoice #: INV-${customer.id}-${Date.now().toString().substr(-6)}`,
          140,
          35
        );
        doc.text(
        ...
        if (customerSales.length === 0) {
          ...
          customerSales.forEach((sale, index) => {
            ...
            doc.text(
              `Farm Fresh Eggs (${sale.dozens.toFixed(1)} dozen)`,
              50,
              yPos
            );
        ...
        // Save the PDF
        doc.save(
          `invoice-${customer.name.replace(/\s+/g, "-").toLowerCase()}.pdf`
        );
      } catch (error) {
  ...
  const handleDownloadPDF = () => {
    ...
    if (typeof window !== "undefined" && window.jspdf) {
      try {
        ...
        // Add summary
        const totalDozens = customers.reduce(
          (sum, customer) => sum + customer.dozensPurchased,
          0
        );
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setNewSale({
                                ...newSale,
                                dozens: parseFloat(e.target.value),
                              })
                            }
                            min="0"
                            step="0.5"
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setNewSale({
                                ...newSale,
                                pricePerDozen: parseFloat(e.target.value),
                              })
                            }
                            min="0"
                            step="0.5"
