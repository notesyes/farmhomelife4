  ...
  const handleGenerateReport = () => {
    ...
    if (typeof window !== 'undefined' && window.jspdf) {
      try {
        ...
        const customerSales = recentSales.filter(sale => sale.customerId === selectedCustomer);
        ...
        if (customerData.email) doc.text(`Email: ${customerData.email}`, 14, 60);
        if (customerData.phone) doc.text(`Phone: ${customerData.phone}`, 14, 65);
        doc.text(`Total Purchases: $${customerData.totalSales.toFixed(2)}`, 14, 70);
        doc.text(`Total Dozens Purchased: ${customerData.dozensPurchased.toFixed(2)}`, 14, 75);
        ...
        if (customerSales.length > 0) {
          doc.text(`Average Purchase Value: $${avgPurchaseValue.toFixed(2)}`, 14, yPos);
        }
        
        // Add footer
        doc.setFontSize(8);
        doc.setTextColor(100, 100, 100);
        doc.text('Farm Home Life - Customer Purchase Report', 105, 280, { align: 'center' });
        
        // Save the PDF
        doc.save(`customer-report-${customerData.name.replace(/\s+/g, '-').toLowerCase()}.pdf`);
      } catch (error) {
        ...
        <div className="flex">
          ...
          <main className="flex-1 p-6">
            <div className="max-w-7xl mx-auto">
              <h1 className="text-2xl font-semibold text-gray-900 mb-6">Customer Purchase Reports</h1>
              
              <div className="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
                <div className="p-6">
                  <h2 className="text-xl font-semibold text-amber-900 mb-4">Generate Customer Purchase Report</h2>
                  <p className="text-gray-600 mb-6">
                    Select a customer to generate a detailed report of their purchase history.
                  </p>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label htmlFor="customerSelect" className="block text-sm font-medium text-amber-700 mb-2">
                        Select Customer
                      </label>
                      <select
                        id="customerSelect"
                      ...
                      >
                        ...
                        {customers.map((customer) => (
                          <option key={customer.id} value={customer.id}>
                            {customer.name} {customer.dozensPurchased > 0 ? `(${customer.dozensPurchased.toFixed(1)} dozens purchased)` : ''}
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div className="flex items-end">
                      <button
                        ...
                        className={`px-4 py-2 rounded-md text-white font-medium transition-colors ${
                          selectedCustomer ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-gray-400 cursor-not-allowed'
                        }`}
                      >
                        Generate PDF Report
                      </button>
                      ...
                      <div className="flex justify-end">
                        <div className="text-right">
                          <p className="text-sm text-amber-800">
                            <span className="font-medium">Total Dozens:</span> {filteredSales.reduce((sum, sale) => sum + sale.dozens, 0).toFixed(2)}
                          </p>
                          <p className="text-sm font-medium text-amber-900 mt-1">
                            <span className="font-medium">Total Amount:</span> ${filteredSales.reduce((sum, sale) => sum + sale.total, 0).toFixed(2)}
                          </p>
                        </div>
                      </div>
                    </div>
