...
function SubscriptionContent() {
  ...
  useEffect(() => {
    const fetchSubscriptionStatus = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            data.message || data.error || "Failed to fetch subscription status."
          );
        }
  ...
  const handleUpdatePaymentMethod = () => {
    ...
    window.open(
      "https://billing.stripe.com/p/login/test_28o5nC8Qs3Yl5LW288",
      "_blank"
    );
  };
  ...
  const handleUpgradeSubscription = async (priceId: string) => {
    if (!priceId || priceId.includes("_xxxx")) {
      // Basic check for placeholder
      setCheckoutMessage(
        "Annual plan Price ID is not configured. Please contact support or check configuration."
      );
      console.error(
        "Annual plan Price ID is a placeholder or missing:",
        priceId
      );
      return;
    }
    ...
    try {
      ...
      let errorMessage =
        "Could not initiate plan upgrade. Please try again later.";
      if (error instanceof Error) {
        errorMessage = error.message;
      }
      setCheckoutMessage(errorMessage);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 mt-8">
              {checkoutMessage && (
                <div
                  className={`mb-6 p-4 rounded-lg text-base ${
                    checkoutMessage.toLowerCase().includes("success") ||
                    checkoutMessage.toLowerCase().includes("activated")
              ...
              {isLoading ? (
                <div className="bg-white shadow rounded-lg p-8 text-center">
                  <svg
                    className="animate-spin h-8 w-8 mx-auto text-emerald-500"
                    ...
                    <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                      <div className="sm:col-span-1">
                        ...
                        <dd className="mt-1 text-sm text-gray-900">
                          Farm Home Life {subscription?.plan}
                        </dd>
                      </div>
                        ...
                        <dd className="mt-1 text-sm text-gray-900 flex items-center">
                          {subscription?.paymentMethod?.brand === "visa" && (
                            ...
                            >
                              ...
                              />
                              <path
                                d="M12.223 7.012H9.451L7.893 12.992H10.665L12.223 7.012Z"
                                fill="#00579F"
                              />
                              <path
                                d="M18.315 7.215C17.784 7.012 16.99 6.797 16.026 6.797C14.098 6.797 12.752 7.797 12.741 9.203C12.73 10.227 13.707 10.801 14.445 11.156C15.205 11.52 15.455 11.754 15.455 12.074C15.444 12.566 14.844 12.789 14.28 12.789C13.505 12.789 13.095 12.684 12.431 12.395L12.145 12.266L11.837 14.617C12.464 14.883 13.639 15.117 14.855 15.129C16.905 15.129 18.218 14.141 18.239 12.629C18.25 11.812 17.752 11.191 16.647 10.664C15.973 10.332 15.562 10.109 15.562 9.766C15.573 9.453 15.919 9.129 16.69 9.129C17.327 9.117 17.784 9.254 18.132 9.398L18.325 9.488L18.632 7.215H18.315Z"
                                fill="#00579F"
                              />
                              <path
                                d="M21.04 7.012C20.565 7.012 20.208 7.16 20.004 7.617L17.232 12.992H19.282L19.67 11.977H22.055L22.27 12.992H24.106L22.483 7.012H21.04ZM20.197 10.367C20.347 9.988 20.885 8.695 20.885 8.695C20.874 8.719 21.019 8.355 21.104 8.129L21.222 8.66C21.222 8.66 21.544 10.035 21.619 10.367H20.197Z"
                                fill="#00579F"
                              />
                              <path
                                d="M8.697 7.012L6.8 10.988L6.622 10.059C6.324 9.027 5.401 7.91 4.361 7.395L6.118 12.98H8.19L11.237 7.012H8.697Z"
                                fill="#00579F"
                              />
                              <path
                                d="M5.508 7.012H2.278L2.246 7.172C4.693 7.777 6.335 9.254 6.958 11.059L6.335 7.629C6.206 7.148 5.897 7.023 5.508 7.012Z"
                                fill="#FAA61A"
                              />
                            </svg>
                          )}
                          •••• {subscription?.paymentMethod?.last4} | Expires{" "}
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="mt-1 text-sm text-gray-500">
                              Best value for serious farmers
                            </p>
                          </div>
                          <div className="text-right">
                            <p className="text-2xl font-bold text-gray-900">
                        ...
                        subscription?.status !== "active" ? (
                          <button
                            ...
                            } // IMPORTANT: Replace with your actual Annual Plan Price ID from Stripe
